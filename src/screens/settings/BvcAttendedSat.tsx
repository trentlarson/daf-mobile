 ...
 * Serto Mobile App
 *
 * copied from CreateCredential.tsx
 *
 */
...
export default () => {
  ...
  let satMorn = moment();
  if (satMorn.weekday() < 6) {
    // it's not Saturday, so let's default to last Saturday
    satMorn.subtract(1, 'week');
  }
  satMorn.weekday(6).hour(9).startOf('hour');
  let satMornStr = satMorn.format();
  ...
  const initialFields = [{
    ...
    value: {
      ...
      startTime: satMornStr
    }

  }]
  const [fields, updateFields] = useState<Field[]>(initialFields)
  ...
  useEffect(() => {
    if (data && data.viewer) {
      ...
      /** must update all fields (this throws away the others)
      updateFields([{
        type: 'agent',
        value: {
          did: data.viewer.did
          ...
          {fields.map((field, i) => {
            return (
              ...
              >
                ...
                <Text textColor={Colors.WHITE}>{JSON.stringify(field.value)}</Text>
              </Container>
            )
          })}
        </Container>
