...
export class DIDComm {
   ...
   */
  public async pack_anon_msg_for_recipients(
    msg: string,
    recipientKeys: Uint8Array[],
  ): Promise<string> {
   ...
   */
  public async pack_nonrepudiable_msg_for_anyone(
    msg: string,
    senderKeys: sodium.KeyPair,
  ): Promise<string> {
  ...
  private async unpackEncrypted(
    ...
    let [cek, senderVk, recipVk] = this.locateRecKey(
      recipsOuter.recipients,
      toKeys,
    )
    ...
    let message = this.decryptPlaintext(
      ciphertext,
      tag,
      wrapper.protected,
      nonce,
    ...
    try {
      ...
      return {
        ...
        senderKey: senderVk,
        nonRepudiableVerification:
          senderVk === jwsVerified.verkey ? true : false,
      }
    } catch (err) {
      return {
        message,
  ...
  private async signContent(
    ...
    let signature = this.b64url(
      sodium.crypto_sign(headerAndPayloadConcat, signerKeyPair.privateKey),
    )
  ...
  private verifyContent(jws: string): JWSUnpacked {
    ...
    let sigMsg = sodium.crypto_sign_open(
      this.b64dec(jwsSplit[2]),
      Base58.decode(joseHeader.kid),
    )
    ...
    return {
      ...
      verified:
        sodium.to_string(sigMsg) === `${jwsSplit[0]}.${jwsSplit[1]}`
          ? true
          : false,
    }
  }
  ...
  private strB64dec(input: any) {
    return this.sodium.to_string(
      this.sodium.from_base64(input, this.sodium.base64_variants.URLSAFE),
    )
  }

  private encryptPlaintext(message: any, addData: any, key: any) {
    let iv = this.sodium.randombytes_buf(
      this.sodium.crypto_aead_chacha20poly1305_ietf_NPUBBYTES,
    )
    let out = this.sodium.crypto_aead_chacha20poly1305_ietf_encrypt_detached(
      message,
      addData,
      null,
      iv,
  ...
  private prepareRecipientKeys(toKeys: any, fromKeys: any = null) {
    ...
    toKeys.forEach((targetVk: any) => {
      ...
      if (fromKeys) {
        ...
        let senderSk = this.sodium.crypto_sign_ed25519_sk_to_curve25519(
          fromKeys.privateKey,
        )
  ...
  private locateRecKey(recipients: any, keys: any) {
    ...
    for (let index in recipients) {
      ...
      let encSender = recip.header.sender
        ? this.b64dec(recip.header.sender)
        : null

      let senderVk = null
      let cek = null
      if (nonce && encSender) {
        senderVk = this.sodium.to_string(
          this.sodium.crypto_box_seal_open(encSender, pk, sk),
        )
        let senderPk = this.sodium.crypto_sign_ed25519_pk_to_curve25519(
          Base58.decode(senderVk),
        )
        cek = this.sodium.crypto_box_open_easy(
          encrytpedKey,
          nonce,
          senderPk,
          sk,
